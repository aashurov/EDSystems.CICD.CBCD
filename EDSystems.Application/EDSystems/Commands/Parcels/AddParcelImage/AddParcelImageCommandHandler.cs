...
public class AddParcelImageCommandHandler : IRequestHandler<AddParcelImageCommand>
{
    ...
    public AddParcelImageCommandHandler(IEDSystemsDbContext dbContext, IWebHostEnvironment env) => (_dbContext, _env) = (dbContext, env);

    public async Task<Unit> Handle(AddParcelImageCommand request, CancellationToken cancellationToken)
    {
