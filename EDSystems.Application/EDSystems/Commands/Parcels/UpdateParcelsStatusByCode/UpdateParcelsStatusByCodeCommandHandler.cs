...
public class UpdateParcelsStatusByCodeCommandHandler : IRequestHandler<UpdateParcelsStatusByCodeCommand>
{
    private readonly IEDSystemsDbContext _dbContext;
    private readonly UserManager<User> _userManager;

    public UpdateParcelsStatusByCodeCommandHandler(IEDSystemsDbContext dbContext, UserManager<User> userManager) => (_dbContext, _userManager) = (dbContext, userManager);

    public async Task<Unit> Handle(UpdateParcelsStatusByCodeCommand request, CancellationToken cancellationToken)
    {
        foreach (var entitymine in request.ParcelCode)
        {
            var entity = await _dbContext.Parcel
                                      ...
                                      .FirstOrDefaultAsync(parcel => parcel.Code == entitymine, cancellationToken);
